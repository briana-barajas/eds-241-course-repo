...
fish %>% 
  pivot_longer(cols = 1:6, names_to = "ecosystem_type",
               values_to = "value") %>% 
  ggplot(aes(x = ecosystem_type, y = value)) +
  geom_col()
```

```{r}
# calculate propensity scores 
prop_scores_fish <- glm(itq ~ met1 + met2 + met3 + met4 + met5 + met6, data = fish,family = binomial())
