...
## Load Data
fish_raw <- read_csv(here("final", "data", "final_fisheries_data.csv")) %>% 
  clean_names()
...
fish_treat <- fish %>% filter(itq == 1)
fish_control <- fish %>% filter(itq == 0)
...
**ANS:** The distribution for ecosystem types in the control and treatment groups (based on `itq`) mirror each other closely, but not perfectly. For example, in `met5` and `met4` there appear to be more observations for the treatment group. 
...
```{r}
propensity_scores <- glm(itq ~ ecosystem_type + comm_val + ind_sr, data = fish, family = binomial())

fish <- fish %>% 
  select(-value) %>% 
  mutate(psvalue = predict(propensity_scores, type = "response"))

histbackback(split(fish$psvalue, fish$itq))
```

**ANS:** Based on the previous questions, the resulting histogram is expected but not ideal for our analysis. There is very little overlap, meaning ...
