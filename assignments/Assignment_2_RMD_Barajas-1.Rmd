...
```{r, include=TRUE, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
# calculate the mean difference in birth weight
birth_weight %>% 
  group_by(tobacco) %>%
...
```
__NOTE:__ Propensity score is the chance of being treated (smoking). For example, the estimate for alcohol is 2.02, this high positive value means that if a mother drinks there's a high chance she also smoked. The negative values for `meduc` means that as mother's education increases, the chance of her smoking decreases.
...
__NOTE:__ The ATT is similar to the measure of ATE we conducted in part 1a, where the difference in means between smoking and non-smoking mothers 222 grams. This value is only slightly smaller than the mean difference in birth weight of unmatched mothers. Since this data was matched, we can see that smoking has a large impact on birth weight even when other factors (education, first, alcohol, etc.) are held constant.
...
## Load the datasets
progresa_pre <- read_csv(here("assignments", "data", "progresa_pre_1997.csv")) 
progresa_post <- read.csv(here("assignments", "data", "progresa_pre_1999.csv")) 
...
```{r, include=TRUE, tidy=TRUE, tidy.opts=list(width.cutoff=60), eval=FALSE}
## Fixed Effects Regression
# remove NAs in treatment colum
progresa_lite <- progresa %>% 
