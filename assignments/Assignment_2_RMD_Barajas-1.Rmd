...
a)  ***What is the mean difference in birth weight of infants with smoking and non-smoking mothers? [1 pt]***
    ...
    ***Calculate and create a table demonstrating the differences in the mean proportions/values of covariates observed in smokers and non-smokers (remember to report whether differences are statistically significant) and discuss whether this provides empirical evidence for or against this assumption.***
    ...
    ```{r}
    ## ========================================
    ##       create subsets of var type   ----
    ## ========================================

    # create df with tobacco and continuous variables
    cont_bw_df <- birth_weight %>% 
      select(c(mage, meduc, tobacco))

    # create df with tobacco and categorical variables
    cat_bw_df <- birth_weight %>% 
      select(-c(mage, meduc, birthwgt))
      
    ## ========================================
    ##       t-test on continuous vars     ----
    ## ========================================

    # list of continuous variables
    cont_vars_bw <- birth_weight %>% 
      select(c(mage, meduc)) %>% 
      names

    # create empty df to store results
    t_test_bw_result <- data.frame()

    # apply t-test to all continuous variables listed
    for (i in cont_vars_bw) {

      # generate t.test formula
      formula <- as.formula(paste(i, "~ tobacco"))

      # apply t.test
      t_test_bw_result <- t.test(formula, data = cont_bw_df)
      
      # tidy df
      t_test_bw_tidy <- broom::tidy(t_test_bw_result)
      t_test_bw_tidy$Variable <-

    }

      # tidy results df, add variable column
      t_test_bw_tidy <- broom::tidy(t_test_bw_result)

      t_test_bw_tidy$Variable <- i

      t_test_bw_table <- rbind(t_test_bw_result, t_test_bw_tidy)
    ```
